...
Beacon (`beacond`) is an audio-based spatial awareness toolkit. It uses two complementary matching strategies: Shazam-style hash-based fingerprinting for precise matching and vector similarity signatures (192-dim spectral envelope with cosine similarity) for ambient sound matching. Zones can be defined by ambient sound signatures or ultrasonic beacons. Everything is self-hosted with no cloud dependencies.
...
4. **`signature.rs`** — Alternative to hash matching: computes a 192-dimensional (64 bands x 3 features) L2-normalized spectral envelope from the spectrogram. Matching uses cosine similarity (dot product on normalized vectors). Signatures are stored as BLOBs in the zones table.

5. **`database.rs`** — SQLite storage via `rusqlite` (bundled). Stores fingerprint hashes mapped to tracks/zones, plus signature BLOBs. Hash matching uses offset histogram alignment. The monitor loop runs both hash and signature matching in parallel, taking the best result.
